...
def solve02(data: str) -> int:
    ...
    for step in steps:
        if '=' in step:
            ...
            if label in boxes[box]:
                boxes[box][label]['val'] = val
            else:
                boxes[box][label] = {
                    'idx': boxes[box]['next_idx'],
                    'val': val
                }
                boxes[box]['next_idx'] += 1
    ...
    for box in boxes:
        ...
        for label in boxes[box]:
            if label != 'next_idx':
                val = int(boxes[box][label]['val'])
                final_boxes[box].append((label, val))
            
    ans = 0
    for box in final_boxes:
        for i in range(len(final_boxes[box])):
            val = final_boxes[box][i][1]
            ans += (box + 1) * (i + 1) * val
    
    return ans

if __name__ == "__main__":
    # data = open('day-15-input.test.txt', 'r').read()
    data = open('day-15-input.txt', 'r').read()
