...
def parse_lines(lines: str) -> dict[str, list[str]]:
    ...
    for line in lines.strip().split('\n'):
        ...
        node = parts[0]
        left_right = [value.strip() for value in parts[1][1:-1].split(',')]
        nodes[node] = left_right
...
if __name__ == "__main__":
    # data = open('day-8-input.test.txt', 'r').read()
    data = open('day-8-input.txt', 'r').read()
